...
|---|---|---|
| `author` | a personal name · `CFI.co Editorial` | **Who is credited, never who posted.** A personal name where a byline has been established (the alignment of 15 August 2026); the house label otherwise, meaning no byline was established — **not** that CFI.co is asserted as the writer. It is not evidence of authoring activity: contributors who hold an account here are posted on their behalf by staff, and staff post on the publisher's behalf, so a WordPress account never establishes who sat down and wrote. Internal staff usernames are never exposed. **Corrected 2026-08-25** — `schema.json` had declared this field a fixed constant `CFI.co Editorial`, which the 15 August byline alignment made untrue for 146 records the same day; they had been failing validation against our own schema since |
...
| `correction_class` | `factual_correction` · `label_regime_change` · `later_development` · `unspecified` | Present **only** when `correction_status` is `revised`, and says which kind. `factual_correction` — the article text itself changed. `label_regime_change` — the text is byte-identical and a claim or the byline changed. `unspecified` — the record was revised before this field existed and nothing in the current run classifies it. `later_development` — the text was accurate when published and carries a dated note because the position it states has since moved. `factual_correction` is one-way and wins, so a record whose text was once corrected can never later present itself as merely re-labelled or as having only been overtaken by events. **Added 2026-08-25** — `correction_status` alone cannot tell "we got the article wrong" from "we changed how we label", and a labelling change across the back catalogue would otherwise leave the field reading `revised` on most of the archive while distinguishing nothing |
| `correction_history` | array of `{date, class}`, oldest first | Present **only** where a record has carried **more than one** correction event; a single event is fully described by `correction_class`. **Added 2026-09-08** — `correction_class` holds one value, so a record that has been through more than one event loses every earlier one. Post 21545 is the case that forced it: a labelling regime change in August 2026, then a factual correction and a later-development note in September, all into a field that holds one value. An array of bare labels would have discarded the dates too, so each entry carries its own |
