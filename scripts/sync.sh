...
#     corpus, so it is committed as one file, not per-article. On days with no
#     record change it is byte-identical and produces no commit. (The `if !`
#     guard keeps `set -e` happy when there IS a diff.)
git add index.jsonl
if ! git diff --cached --quiet -- index.jsonl; then
  git "${ID[@]}" commit -q --no-verify -m "Refresh article index ($(date -u +%F))"
