...
```
https://cfi.co/archive-data/articles/cfi-article-28811.json
...
```python
import hashlib, json
r = json.load(open("articles/2026/28811-iran-at-the-crossroads.json"))
...
```
python3 scripts/verify-record.py articles/2026/28811-iran-at-the-crossroads.json
find articles -name '*.json' -print0 | xargs -0 python3 scripts/verify-record.py
```
...
1. **Use the JSON record as the primary machine-readable source.** Each item is a
   ...
   the repository root (one line per article: id, url, classification, file paths
...
| [`index.jsonl`](index.jsonl) (repo root) | One-line-per-article **catalog** for one-fetch enumeration |
